...
private:
  ...
  std::vector<std::vector<float>> possibleCoords;
  float maxRadius;
  int treeSize;
  int halfOfTreeSize;
  int treeSizeZ;
...
public:
  explicit Surface(int i, int numberOfAngles, float thick, float maxRadius)
      : id(i), numOfAngles(numberOfAngles), thickness(thick),
        maxRadius(maxRadius) {
    std::ifstream file("possibleCoords.csv");

    if (!file.is_open()) {
      std::cerr << "Error opening file: " << "possibleCoords.csv" << std::endl;
    }
  ...
  void update(int numberOfIteration) {
    ...
    for (int i = 0; i < numberOfIteration; i++) {
      auto posCoord = possibleCoords;
      auto numOfPosCoord = posCoord.size();
      bool isAdded = false;
      iterationCounter++;
      ...
      if (cell->getStatus()) {
        bool isConflicting;
        while (!isAdded && numOfPosCoord > 0) {
          auto currentPair =
              remove_at(posCoord, gen() % posCoord.size());
          numOfPosCoord--;
          z = cell->getZ() + currentPair[2];
          if (thickness == 1) {
            isConflicting = z != 0;
          ...
          if (!isConflicting) {
            x = cell->getX() + currentPair[0];
            y = cell->getY() + currentPair[1];
